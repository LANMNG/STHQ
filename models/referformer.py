...
class ReferFormer(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, 
        ...
        self.class_embed_sthq = nn.Linear(hidden_dim, num_classes)
        ...
        self.class_embed_sthq.bias.data = torch.ones(num_classes) * bias_value
        ...
        if with_box_refine:
            ...
            self.class_embed_sthq = _get_clones(self.class_embed_sthq, 2)
        ...
        self.sthq_module = STHQ(in_channels=hidden_dim, hidden_dim=hidden_dim,num_frames=num_frames,
                                num_frame_queries=num_queries, num_queries= num_queries, aux_loss=aux_loss)
        ...
        weight_nums_sthq, bias_nums_sthq = [], []
        for l in range(self.controller_layers):
            if l == 0:
                weight_nums_sthq.append(self.in_channels * self.dynamic_mask_channels)
                bias_nums_sthq.append(self.dynamic_mask_channels)
            elif l == self.controller_layers - 1:
                weight_nums_sthq.append(self.dynamic_mask_channels * 1)  # output layer c -> 1
                bias_nums_sthq.append(1)
            else:
                weight_nums_sthq.append(self.dynamic_mask_channels * self.dynamic_mask_channels)
                bias_nums_sthq.append(self.dynamic_mask_channels)

        self.weight_nums_sthq = weight_nums_sthq
        self.bias_nums_sthq = bias_nums_sthq
        self.num_gen_params_sthq = sum(weight_nums_sthq) + sum(bias_nums_sthq)

        self.controller_sthq = MLP(hidden_dim, hidden_dim, self.num_gen_params_sthq, 3)
        for layer in self.controller_sthq.layers:
            nn.init.zeros_(layer.bias)
            nn.init.xavier_uniform_(layer.weight)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        sthq_outputs = self.sthq_module(hs,t) # # D, L, B, cQ, C   [3,4,1,7,256]
        ...
        sthq_outputs = sthq_outputs[-1]  # L, B, cQ, C   [4,1,7,256]
        sthq_outputs = repeat(sthq_outputs, 'l b q c -> l b t q c', t=t)
        sthq_outputs = rearrange(sthq_outputs, 'l b t q c -> l (b t) q c', b=b, t=t)

        outputs_seg_masks_sthq = []
        outputs_classes_sthq = []
        for lvl in range(sthq_outputs.shape[0]):
            if lvl >1:
                dynamic_mask_head_params_sthq = self.controller_sthq(sthq_outputs[lvl])  # [batch_size*time, num_queries_per_frame, num_params]
                dynamic_mask_head_params_sthq = rearrange(dynamic_mask_head_params_sthq, '(b t) q n -> b (t q) n', b=b, t=t)
                outputs_seg_mask_sthq = self.dynamic_mask(mask_features, dynamic_mask_head_params_sthq)
                outputs_seg_mask_sthq = rearrange(outputs_seg_mask_sthq, 'b (t q) h w -> b t q h w', t=t)
                outputs_seg_masks_sthq.append(outputs_seg_mask_sthq)
                outputs_class_sthq = self.class_embed_sthq[lvl-2](sthq_outputs[lvl])
                outputs_classes_sthq.append(outputs_class_sthq)

        outputs_class_sthq = torch.stack(outputs_classes_sthq)
        outputs_class_sthq = rearrange(outputs_class_sthq, 'l (b t) q k -> l b t q k', b=b, t=t)
        out['pred_logits'] = outputs_class_sthq[-1]
        out['pred_masks'] = outputs_seg_masks_sthq[-1]  # [batch_size, time, num_queries_per_frame, out_h, out_w]

        if self.aux_loss:
            outputs_seg_masks_aux = [outputs_seg_masks[-2], outputs_seg_masks[-1], outputs_seg_masks_sthq[-2]]
            outputs_coord_aux = [outputs_coord[-2], outputs_coord[-1], outputs_coord[-2]]
            outputs_class_aux = [outputs_class[-2], outputs_class[-1], outputs_class_sthq[-2]]
            out['aux_outputs'] = self._set_aux_loss(outputs_class_aux, outputs_coord_aux, outputs_seg_masks_aux)
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_seg_masks):
        ...
        return [{"pred_logits": a, "pred_boxes": b, "pred_masks": c} 
                for a, b, c in zip(outputs_class, outputs_coord, outputs_seg_masks)]
    ...
    def dynamic_mask(self, mask_features, mask_head_params):
        ...
        b, t, c, h, w = mask_features.shape
        # this is the total query number in all frames
        _, num_queries = mask_head_params.shape[:2]
        q = num_queries // t  # num_queries_per_frame
        ...
        weights, biases = parse_dynamic_params(
            mask_head_params, self.dynamic_mask_channels,
            self.weight_nums_sthq, self.bias_nums_sthq
        )
